...
func (i *ProxyTcp) Handle() {
	...
	if err != nil {
		Log.Log.Fatal("[-] 解析tcp代理目标地址错误：" + err.Error())
		return
	}
	...
	if err != nil {
		Log.Log.Fatal("[-] 连接tcp代理目标地址错误：" + err.Error())
		return
	}
	// 处理tls握手
	...
	if err != nil {
		Log.Log.Fatal("[-] " + conn.RemoteAddr().String() + "：获取证书失败：" + err.Error())
		return
	}
	...
	err = <-stop
	Log.Log.Fatal("[-] 转发tcp数据错误：" + err.Error())
}
