...
import (
	...
	"GoRuleMatch/main/ShermieProxy/Core/Websocket"
	"GoRuleMatch/main/ShermieProxy/Utils"
	"bufio"
	"bytes"
	"compress/gzip"
	"context"
...
func (i *ProxyHttp) Handle() {
	...
	if err != nil {
		Log.Log.Fatal("[-] 读取请求错误：" + err.Error())
		return
	}
	i.port = "-1"
...
func (i *ProxyHttp) handleRequest() {
	...
	if i.response == nil {
		Log.Log.Fatal("[-] 远程服务器无响应-1")
		return
	}
	if err != nil {
		Log.Log.Fatal("[-] 获取远程服务器响应失败：" + err.Error())
		return
	}
...
func (i *ProxyHttp) Transport(request *http.Request) (*http.Response, error) {
	...
	if err != nil {
		Log.Log.Fatal("[-] 转发http请求失败：" + err.Error())
		return nil, err
	}
...
func (i *ProxyHttp) handleSslRequest() {
	...
	if err != nil {
		Log.Log.Fatal("[-] 返回连接状态失败：" + err.Error())
		return
	}
	// 建立TLS连接并返回给源
	i.SslReceiveSend()
...
func (i *ProxyHttp) SslReceiveSend() {
	...
	if err != nil {
		Log.Log.Fatal("[-] " + i.request.Host + "：获取证书失败：" + err.Error())
		return
	}
	...
	if err != nil {
		...
		if err == io.EOF || strings.Index(err.Error(), "closed") != -1 {
			Log.Log.Fatal("[-] 客户端TLS握手失败：" + err.Error())
			return
		}
	...
	if err != nil {
		if err == io.EOF {
			Log.Log.Fatal("[-] 浏览器TLS连接断开：" + err.Error())
			return
		}
		Log.Log.Fatal("[-] 读取TLS连接请求数据失败：" + err.Error())
		return
	}
...
func (i *ProxyHttp) handleWsHandshakeErr(rawProtocolInput []byte) {
	...
	for _, value := range rawInputList {
		...
		if headerKeValList[0] == "Host" {
			...
			if err != nil {
				Log.Log.Fatal("[-] 解析ws请求地址错误：" + err.Error())
				return
			}
		}
...
func (i *ProxyHttp) handleWsRequest() bool {
	...
	if err != nil {
		Log.Log.Fatal("[-] 升级ws协议失败：" + err.Error())
		return true
	}
	...
	if err != nil {
		...
		Log.Log.Fatal("[-] 连接ws服务器失败：" + string(header) + err.Error())
		return true
	}
	defer func() {
