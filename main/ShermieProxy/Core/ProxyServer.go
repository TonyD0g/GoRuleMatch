...
import (
	...
	"GoRuleMatch/main/ShermieProxy/Contract"
...
func (i *ProxyServer) Install() {
	if runtime.GOOS == "windows" {
		...
		Log.Log.Fatal("[+] 已设置系统代理")
		return
	}
	Log.Log.Fatal("[+] 非windows系统请手动安装证书并设置代理,可以在根目录或访问http://127.0.0.1/tls获取证书文件")
}
...
func (i *ProxyServer) Start() error {
	i.beforeStart()
	Log.Log.Println("[+] 监听的 ip + 端口 :0.0.0.0:" + i.port)
...
func (i *ProxyServer) handle(conn net.Conn) {
	var process Contract.IServerProcessor
	defer func() {
