...
// Init 初始化根证书
func (i *Certificate) Init(certFile, keyFile string) error {
	var err error
	var certBlock, keyBlock *pem.Block
	// 如果根证书不存在,则生成
	if !Utils.FileExist(certFile) {
		// 生成根pem文件
		certBlock, keyBlock, err = i.GenerateRootPemFile("GoRuleMatch")
		if err != nil {
...
func (i *Certificate) GenerateRootPemFile(host string) (*pem.Block, *pem.Block, error) {
	...
	template := x509.Certificate{
		...
		KeyUsage:              x509.KeyUsageKeyEncipherment | x509.KeyUsageDigitalSignature | x509.KeyUsageCertSign,
		ExtKeyUsage:           []x509.ExtKeyUsage{x509.ExtKeyUsageServerAuth},
		EmailAddresses:        []string{"1238230139@gmail.com"},
		BasicConstraintsValid: true,
		IsCA:                  true,
		Issuer: pkix.Name{
			CommonName: host,
