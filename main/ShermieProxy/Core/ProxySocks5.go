...
func (i *ProxySocks5) Handle() {
	...
	if err != nil {
		Log.Log.Fatal("[-] 读取socks5版本号错误：" + err.Error())
		return
	}
	if version != Version {
		Log.Log.Fatal("[-] socks5版本号不匹配")
		return
	}
	// 读取支持的方法
	methodNum, err := i.reader.ReadByte()
	if err != nil {
		Log.Log.Fatal("[-] 读取socks5支持方法数量错误：" + err.Error())
		return
	}
	if methodNum < 0 || methodNum > 0xFF {
		Log.Log.Fatal("[-] socks5支持方法参数错误")
		return
	}
	// 代理默认不需要账号密码验证
	...
	for n := 0; n < int(methodNum); n++ {
		...
		if err != nil {
			Log.Log.Fatal("[-] 读取socks5支持错误：" + err.Error())
			return
		}
	}
	_, err = i.writer.Write([]byte{Version, 0x00})
	if err != nil {
		Log.Log.Fatal("[-] 返回数据错误：" + err.Error())
		return
	}
	_ = i.writer.Flush()
	if requiredAuth {
	...
	if err != nil {
		Log.Log.Fatal("[-] 读取端口号错误：" + err.Error())
		return
	}
	...
	err = i.writer.Flush()
	if err != nil {
		Log.Log.Fatal("[-] 写入socks5握手错误：" + err.Error())
		return
	}
	...
	err = <-out
	Log.Log.Fatal("[-] 代理socks5数据错误：" + err.Error())
}
